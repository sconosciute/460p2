...
const validTitle = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.query.title || validationFunctions.isStringProvided(req.query.title)) {
        next();
    } else {
        res.status(400).send({
...
const validAuthor = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.query.author || validationFunctions.isStringProvided(req.query.author)) {
        next();
    } else {
        res.status(400).send({
...
const validMinMax = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.query.min || !req.query.max || (isNumberProvided(req.query.min) && isNumberProvided(req.query.max))) {
